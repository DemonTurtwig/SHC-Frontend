...
export default function BookingSubtypeSelect() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await axios.get(`${API}/booking/initialize`);
    ...
    <LinearGradient colors={['#d0eaff', '#89c4f4']} style={styles.wrapper}>
      <View style={styles.header}>
        <TouchableOpacity onPress={handleBack} style={styles.backButton}>
          <Image
            source={require('./asset/icons/back-button.png')}
          ...
          >
            {subtypes.length === 0 ? (
              ...
              subtypes.map(sub => (
                ...
                >
                  <Image
                    source={subtypeIcons[sub.name] ?? require('./asset/icons/byukgulyee-icon.png')}
