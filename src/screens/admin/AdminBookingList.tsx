...
export default function AdminBookingList() {
  ...
  const saveStatus = async (newStatus: Booking['status']) => {
    ...
    try {
      ...
      await axios.patch(
        `${API}/admin/bookings/${editing._id}/status`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      setEditing(null);
  ...
  <View style={styles.card}>
    ...
    <View style={{ position: 'absolute', right: 14, top: 10, flexDirection: 'row', gap: 8 }}>
      <TouchableOpacity onPress={() => { setEditing(item); setPrice(item.totalPrice.toString()); }}>
        <Image source={require('.././asset/icons/edit.png')} style={styles.editIcon} />
      </TouchableOpacity>
      <TouchableOpacity onPress={() => deleteBooking(item._id)}>
        <Image source={require('.././asset/icons/delete-booking.png')} style={[styles.editIcon, { tintColor: '#d32f2f' }]} />
      </TouchableOpacity>
        </View>
      </View>
  </TouchableOpacity>
      ...
      <View style={styles.footer}>
        ...
        <View style={[styles.footerItem, styles.activeTab]}>
          <Image source={require('.././asset/icons/view-order.png')} style={[styles.footerIcon, styles.activeIcon]} />
